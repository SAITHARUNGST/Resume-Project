...
const workExperienceHtml = userJobExperiences.map(experienceItem => {
  const companyName = experienceItem.querySelector('textarea[name]').value;
  const jobDescription = experienceItem.querySelector('textarea[name]').value;
...
for (let i = 0; i < numWorkExperiences; i++) {
    ...
    companyNameTextarea.name = `companyName${i + 1}`;
    ...
    jobDescriptionTextarea.name = `jobDescription${i + 1}`;
